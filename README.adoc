...
# Start up the client compiler server:
yarn dev
```

=== The client build server
Load the http://localhost:9630[build manager] (http://localhost:9630)

There you can kick off the compilation of the builds and then open their pages in a browser
...
1. Open and connect to nREPL at localhost:9000.
...
You will want to enable the `:dev` Clojure dependency while developing this project.  In IntelliJ this is in the
