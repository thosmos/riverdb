...
```
unzip datomic-pro-0.9.6397.zip
cd datomic-pro-0.9.6397
...
```
APP_ENV=dev
DATOMIC_URI=datomic:dev://localhost:4334/riverdb
PORT=8989
```

run the server:
```
clojure -A:dev:server:rad:nrepl:run
```
...
Run the shadow-cljs build server with:
...
You will want to enable the `:dev`, `:server`, and `:rad` Clojure dependencies while developing the server-side of this project.  In IntelliJ this is in the
